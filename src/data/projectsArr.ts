export const projects = [
    {
        ...
        desc: 'Один из моих самых первых "серьезных" проектов. Мне понравилось его делать, с ним я впервые осваивал Typescript.' +
            ' Тут реализована фильтрация через URL, корзина, а так же можно выбрать тип определенной пиццы',
        id: 1,
    ...
    },
    {
        ...
        desc: 'Этот проект куда серьезней того, что я делал до этого, тк тут работает связка всех фильтров на сайте,' +
            ' причем не через URL. Так же реализована панель админа, можно удалять, редактировать или создавать карточки.' +
            ' Все созданные карточки сохраняются в LocaleStorage. Данные придумывал сам',
        id: 2,
    ...
    },
    {
        ...
        desc: 'Самый крупный проект в моей карьере. Реализована вся часть оригинального сайта, работает с БД на основе кинопоиска,' +
            ' Разрабатывал проект не один. Фильтрация, отрисовка,' +
            ' авторизация - все работает через бекенд, на сайте можно зарегестрироваться. Регистрация реализована через JWT токены.' +
            ' Проект был так же прогнан через множества тестов,' +
            ' реализована StoryBook. К сожалению, деплоя нет, ибо мои бекендеры отказались делать деплой',
        id: 3,
    ...
    },
    {
        ...
        desc: 'Мини магазин с мемным котом нямушка. Логика простая, работают ховеры, эктивы. ' +
            'Реагирует в любых случаях и подставляет разные данные в зависимости от того, что нужно',
        id: 5,
    ...
    },
    {
        ...
        desc: 'Этот проект я решил написать на новой для себя технологии и это - Astro.' +
            ' Делая эту работу мне он понравился, особенно если сравнивать его с NextJS, ведь они похожи.' +
            ' По скорости разработки многократно превосходит Next.' +
            ' Впрочем, отдаю предпочтение этому фреймворку и, надеюсь, продолжу его использовать и дальше :) Так же на этом ' +
            'проекте освоил Taiwind и множество разных библиотек',
        id: 6,
    ...
    },
    {
        ...
        desc: 'Простенькое приложение для оставление комментариев. Форма абсолютно валидная, нельзя не ' +
            'заполнить что-то, так же нельзя отправлять пустые сообщения. В случае не указания даты - указывается сегодняшняя.' +
